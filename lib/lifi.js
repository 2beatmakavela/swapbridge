 ...
 */
export async function fetchLiveChains() {
  const now = Date.now();
 ...
 * Throws on failure — callers should catch and fall back to mock data.
 */
export async function fetchLiveTokens(chainIdOrIds) {
  ...
  const request = (async () => {
    const data = await get('/tokens', { chains: chainIdOrIds });
